...
def export_data(filename):
    ...
    with open(filename, "a", encoding="utf-8") as data: 
        ...
        phone_number = input("Введите number phone: ")
        data.write(f"{num} | {fio} | {phone_number}\n")
        print(f"Доб - зап : {num} | {fio} | {phone_number}\n")
...
def edit_data(filename):
    ...
    print(tel_book)
    print("")
    index_delete_data = int(input("Введите номер строки редактирования: ")) - 1
    tel_book_lines = tel_book.split("\n")
    edit_tel_book_lines = tel_book_lines[index_delete_data]
    elements = edit_tel_book_lines.split(" | ")
    fio = input("Введите FIO: ")
    phone = input("Введите number phone: ")
    num = elements[0]
    if len(fio) == 0:
        fio = elements[1]
    if len(phone) == 0:
    ...
    print(f"Строка - {edit_tel_book_lines}, изменена - {edited_line}\n")
...
def delete_data(filename):
    ...
    with open(filename, "r", encoding="utf-8") as data:
        ...
        print(tel_book)
    print("")
    index_delete_data = int(input("Введите номер строки удаления: ")) - 1
    tel_book_lines = tel_book.split("\n")
    del_tel_book_lines = tel_book_lines[index_delete_data]
    tel_book_lines.pop(index_delete_data)
    print(f"Удалена строка: {del_tel_book_lines}\n")
